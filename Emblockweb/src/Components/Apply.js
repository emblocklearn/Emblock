...
//     } else {
//       setRedirectPath('/frontend-form'); // Set the redirect path to FrontendForm
//       toast.info('Please log in before applying.'); // Display toast notification
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        user.getIdToken(true).then((token) => {
          // Verify token logic (You may want to call a function to check this token on your backend)
          // For now, we'll assume the token is valid if the user is logged in
  ...
  return (
    <div className="relative flex flex-col w-full h-screen md:h-auto  md:flex-row justify-between items-center bg-gray-100 p-6 md:p-10 rounded-lg shadow-lg max-w-[66rem] overflow-auto ">
      ...
      <div className="w-full flex flex-col items-center md:items-start space-y-4">
        ...
        <div>
          <div className='flex flex-col '>
            <h1 className="text-2xl pt-[12px] md:text-3xl font-bold text-green-600 text-center md:text-left">{offer.title}</h1>
            <div className="w-full block  md:hidden md:w-[60%] mt-8 md:mt-0 flex justify-center">
              <div className="relative flex flex-col items-center">
                <img
                  src={offer.imgSrc}
